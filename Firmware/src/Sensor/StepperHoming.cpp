                         ...
                         bool seekToMin)
{
  ...
  const double backoffTarget = enc.angle(Encoder::Radians) + (backoffDir * std::fabs(_cfg.backoffOffset));
  ...
  while (true)
  {
    ...
    const double error = std::fabs(enc.angle(Encoder::Radians) - backoffTarget);
