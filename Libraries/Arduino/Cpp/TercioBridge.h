...
private:
  ...
  // Caches
  struct AxisSlot { uint16_t id=0xFFFF; AxisState st; bool valid=false; };
  struct ImuSlot  { uint16_t id=0xFFFF; ImuState  st; bool valid=false; };
...
public:
  ...
  bool set_can_id(uint16_t new_id) {
    ...
    if (ok) { _id = new_id; _b.registerAxis(_id); }
    return ok;
  }
...
public:
  ...
  bool set_can_id(uint16_t new_id) {
    ...
    if (ok) { _id = new_id; _b.registerImu(_id); }
    return ok;
  }
